...
export function LidPreview({ design }: { design: LidDesign }) {
  ...
  useEffect(() => {
    ...
    const maxDimension = Math.max(design.diameterMm, design.widthMm, design.heightMm);
    ...
    for (const geometry of createLidGeometries(design)) {
      model.add(
        new THREE.Mesh(
          geometry,
          new THREE.MeshStandardMaterial({ color: "#2f7c6b", roughness: 0.58, side: THREE.DoubleSide }),
        ),
      );
    }
  ...
  return <div className="preview-3d interactive-preview" ref={hostRef} aria-label="3D lid preview" />;
}
